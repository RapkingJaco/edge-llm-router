...
原本是無限循環的即時模擬，觀眾看不懂「數字為何一直變」。改成**可控三態**，清楚十倍：
- **idle（靜止）**：一進來不跑，提示「選請求數、按 ▶ 開始」。
- **running**：選 100/200/300/500（可勾尖峰）→ `{"cmd":"run","n":N}` → 跑剛好 N 筆、進度條前進。
- **finished**：跑到底自動停，跳出**統計視窗**（AI vs 傳統：處理/丟棄/TTFT/成本/總 reward + 領先%）。
- 後端 `LiveSimulation` 變狀態機（`start_run(n)` / `reset()`），`sim/workload.generate_n()` 產剛好 N 筆；
  抽驗只在 running 時跑。實測：跑 100 筆 → 統計視窗「AI 贏 ▲37.9%、成本 0.57 vs 1.00」。
